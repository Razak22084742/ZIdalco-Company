...
router.get('/notifications', async (req, res) => {
  try {
    const [recentFeedback, recentEmails] = await Promise.all([
      supabaseRequest('feedback?status=neq.deleted&order=created_at.desc&limit=10', 'GET'),
      supabaseRequest('emails?status=neq.deleted&order=created_at.desc&limit=10', 'GET')
    ]);
    ...
    if (recentEmails.status === 200) {
      for (const em of recentEmails.data) {
        notifications.push({
          type: 'email',
          id: em.id,
          title: `Email from ${em.sender_name}`,
...
router.post('/reply-feedback', async (req, res) => {
  try {
    ...
    await supabaseRequest(`feedback?id=eq.${feedback_id}`, 'PATCH', { 
      status: 'replied',
      is_read: true
    });

    // Try to send email notification to the original feedback sender
    const feedbackData = feedback.data[0];
    if (feedbackData.email && feedbackData.email !== 'unknown@local') {
      try {
        await sendEmail({
          sender_name: 'Zidalco Admin',
          sender_email: process.env.SMTP_FROM || 'no-reply@zidalco.com',
          sender_phone: '',
          message: `Thank you for your feedback! Here's our reply:\n\n${reply_message}`,
          recipient_email: feedbackData.email
        });
      } catch (emailError) {
        console.log('Email notification failed (this is okay):', emailError.message);
      }
    }
...
router.get('/feedback/:id', async (req, res) => {
  try {
    const { id } = req.params;
    console.log('Admin feedback/:id route hit with ID:', id);
    ...
    if (result.status === 200 && Array.isArray(result.data) && result.data.length > 0) {
      ...
      console.log('Feedback not found, returning 404');
